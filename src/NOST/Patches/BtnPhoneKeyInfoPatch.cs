...
namespace NOST.Patches
{
    ...
    public class BtnPhoneKeyInfoPatch
    {
        public static Boolean Prefix(ref Form1 __instance)
        {
            ...
            p.StartInfo.FileName = Path.Combine(Path.GetDirectoryName(typeof(Program).Assembly.Location), "fastboot-android.exe");
            ...
            {
                MethodInfo showErrorMessage = typeof(Form1).GetMethods(BindingFlags.Instance | BindingFlags.NonPublic)
                    .FirstOrDefault(m => m.Name == "ShowErrorMessage" && m.GetParameters().Select(pa => pa.ParameterType).Contains(typeof(Int64)));
                showErrorMessage.Invoke(__instance,
                    new Object[] {50723, null});
                
                // Disable the next button, it does nothing
                FieldInfo btnNext = typeof(Form1).GetField("btnNext", BindingFlags.Instance | BindingFlags.NonPublic);
